...
class UcsSystemConfig(GenericUcsConfig):
    ...
    def _fetch_sdk_objects(self, force=False):
        ...
        sdk_objects_to_fetch = [
            ...
            'adaptorQual', 'adaptorRssProfile', 'biosVProfile', 'callhomeAnonymousReporting', 'callhomeDest',
            'callhomeEp', 'callhomePeriodicSystemInventory', 'callhomePolicy', 'callhomeProfile', 'callhomeSmtp',
            'callhomeSource', 'cimcvmediaConfigMountEntry', 'cimcvmediaMountConfigDef', 'cimcvmediaMountConfigPolicy',
            'commCimcWebService', 'commCimxml', 'commDateTime', 'commDns', 'commDnsProvider', 'commHttp', 'commHttps',
            'commShellSvcLimits', 'commSnmp', 'commSnmpTrap', 'commSnmpUser', 'commSsh', 'commSyslog',
            'commSyslogClient', 'commSyslogConsole', 'commSyslogFile', 'commSyslogMonitor', 'commSyslogSource',
            'commTelnet', 'commWebSvcLimits', 'computeBoard', 'computeChassisDiscPolicy', 'computeChassisQual',
            'computeFanPolicy', 'computeGraphicsCardPolicy', 'computeHwChangeDiscPolicy', 'computeKvmMgmtPolicy',
            'computeMemoryConfigPolicy', 'computeModularChassisFanPolicy', 'computePhysicalQual', 'computePool',
            'computePooledRackUnit', 'computePooledSlot', 'computePoolingPolicy', 'computePortDiscPolicy',
            'computePowerExtendedPolicy', 'computePowerSyncPolicy', 'computePsuPolicy', 'computeQual',
            'computeRackQual', 'computeScrubPolicy', 'computeServerDiscPolicy', 'computeServerMgmtPolicy',
            'computeSlotQual', 'cpmaintMaintPolicy', 'diagMemoryTest', 'diagRunPolicy', 'dpsecMac', 'epqosDefinition',
            'epqosEgress', 'equipmentBinding', 'equipmentChassisProfile', 'equipmentComputeConnPolicy', 'etherPIo',
            'fabricBreakout', 'fabricDceSwSrvEp', 'fabricDceSwSrvPcEp', 'fabricEthEstcEp', 'fabricEthEstcPc',
            'fabricEthEstcPcEp', 'fabricEthLanEp', 'fabricEthLanFlowMonitoring', 'fabricEthLanPc', 'fabricEthLanPcEp',
            'fabricEthLinkProfile', 'fabricEthMon', 'fabricEthMonDestEp', 'fabricEthMonSrcRef', 'fabricEthMonSrcEp',
            'fabricEthTargetEp', 'fabricEthVlanPc', 'fabricEthVlanPortEp', 'fabricFcEndpoint', 'fabricFcEstcEp',
            'fabricFcMon', 'fabricFcMonSrcEp', 'fabricFcMonSrcRef', 'fabricFcSan', 'fabricFcSanEp', 'fabricFcSanPc',
            'fabricFcSanPcEp', 'fabricFcUserZone', 'fabricFcVsanPc', 'fabricFcVsanPortEp', 'fabricFcZoneProfile',
            'fabricFcoeEstcEp', 'fabricFcoeSanEp', 'fabricFcoeSanPc', 'fabricFcoeSanPcEp', 'fabricFcoeVsanPortEp',
            'fabricLacpPolicy', 'fabricLanCloud', 'fabricLanCloudPolicy', 'fabricLanPinGroup', 'fabricLanPinTarget',
            'fabricMulticastPolicy', 'fabricNetGroup', 'fabricNetGroupRef', 'fabricOrgVlanPolicy', 'fabricPooledVlan',
            'fabricReservedVlan', 'fabricSanCloud', 'fabricSanPinGroup', 'fabricSanPinTarget', 'fabricUdldLinkPolicy',
            'fabricUdldPolicy', 'fabricVCon', 'fabricVConProfile', 'fabricVlan', 'fabricVlanGroupReq', 'fabricVlanReq',
            'fabricVsan', 'faultPolicy', 'fcPIo', 'fcpoolBlock', 'fcpoolInitiators', 'firmwareAutoSyncPolicy',
            ...
            'lstorageDiskZoningPolicy', 'lstorageLocal', 'lstorageLocalDiskConfigRef', 'lstorageLogin',
            'lstorageLunSetConfig', 'lstorageProfile', 'lstorageProfileBinding', 'lstorageProfileDef', 'lstorageRemote',
            'lstorageSasExpanderConfigPolicy', 'lstorageVirtualDriveDef', 'macpoolBlock', 'macpoolPool',
            ...
            'powerGroupQual', 'powerMgmtPolicy', 'powerPolicy', 'processorQual', 'qosclassEthBE',
            'qosclassEthClassified', 'qosclassFc', 'qosclassSlowDrain', 'solConfig', 'solPolicy',
            'statsThr32Definition', 'statsThr32Value', 'statsThr64Definition', 'statsThr64Value',
            'statsThrFloatDefinition', 'statsThrFloatValue', 'statsThresholdClass', 'statsThresholdPolicy',
            'storageConnectionPolicy', 'storageFcTargetEp', 'storageIniGroup', 'storageInitiator',
            'storageLocalDiskConfigDef', 'storageLocalDiskConfigPolicy', 'storageQual', 'storageVsanRef',
            'sysdebugBackupBehavior', 'sysdebugMEpLogPolicy', 'topInfoPolicy', 'topInfoSyncPolicy', 'uuidpoolBlock',
            'uuidpoolPool', 'vnicConnDef', 'vnicDynamicCon', 'vnicDynamicConPolicy', 'vnicDynamicConPolicyRef',
            'vnicEther', 'vnicEtherIf', 'vnicFc', 'vnicFcGroupDef', 'vnicFcIf', 'vnicFcNode', 'vnicIPv4Dhcp',
            'vnicIPv4If', 'vnicIPv4IscsiAddr', 'vnicIpV4PooledAddr', 'vnicIPv4PooledIscsiAddr', 'vnicIpV4StaticAddr',
            'vnicIpV6StaticAddr', 'vnicIScsi', 'vnicIScsiAutoTargetIf', 'vnicIScsiBootParams', 'vnicIScsiBootVnic',
            'vnicIScsiLCP', 'vnicIScsiNode', 'vnicIScsiStaticTargetIf', 'vnicIpV4MgmtPooledAddr',
            'vnicIpV6MgmtPooledAddr', 'vnicLanConnPolicy', 'vnicLanConnTempl', 'vnicLun', 'vnicSanConnPolicy',
            'vnicSanConnTempl', 'vnicUsnicConPolicy', 'vnicUsnicConPolicyRef', 'vnicVhbaBehPolicy', 'vnicVlan',
            'vnicVmqConPolicy', 'vnicVmqConPolicyRef', 'vnicVnicBehPolicy'] + bios_sdk_objects_to_fetch
...
class UcsCentralConfig(GenericUcsConfig):
    ...
    def _fetch_sdk_objects(self, force=False):
        ...
        sdk_objects_to_fetch = [
            ...
            'commSyslogMonitor', 'commSyslogSource', 'commTelnet', 'commWebSvcLimits', 'computeBoard', 'computeChassisDiscPolicy',
            'computeChassisQual', 'computeDomainGroupQual', 'computeDomainHwChangeDiscPolicy', 'computeDomainNameQual',
            'computeDomainPortDiscPolicy', 'computeDomainQual', 'computeGraphicsCardPolicy', 'computeGroupMembership',
            'computePhysicalQual', 'computePool', 'computePoolingPolicy', 'computePooledRackUnit', 'computePooledSlot',
            'computeOwnerQual', 'computePowerSyncPolicy', 'computeProductFamilyQual', 'computePsuPolicy', 'computeQual',
            'computeRackQual', 'computeScrubPolicy', 'computeServerDiscPolicy', 'computeServerMgmtPolicy',
            'computeSiteQual', 'computeSlotQual', 'computeSystem', 'computeSystemAddrQual', 'computeSystemQual',
            'dpsecMac', 'epqosDefinition', 'epqosEgress', 'equipmentBinding', 'equipmentChassisProfile',
            'equipmentComputeConnPolicy', 'fabricEthLinkProfile', 'fabricLacpPolicy', 'fabricLanCloudPolicy',
            'fabricMulticastPolicy', 'fabricNetGroup', 'fabricNetGroupRef', 'fabricNetGroupReq', 'fabricPooledVlan',
            'fabricUdldLinkPolicy', 'fabricVCon', 'fabricVConProfile', 'fabricVlan', 'fabricVlanReq', 'fabricVsan',
            'fcpoolBlock', 'fcpoolInitiators', 'firmwareAutoSyncPolicy', 'firmwareChassisPack',
            'firmwareComputeHostPack', 'firmwareExcludeChassisComponent', 'firmwareExcludeServerComponent',
            'flowctrlItem', 'identpoolBlockQual', 'identpoolDomainGroupQual', 'inbandPolicy', 'ippoolBlock',
            'ippoolIpV6Block', 'ippoolPool', 'iqnpoolBlock', 'iqnpoolPool', 'iscsiAuthProfile', 'lsBinding',
            'lsbootBootSecurity', 'lsbootDef', 'lsbootDefaultLocalImage', 'lsbootEFIShell',
            'lsbootEmbeddedLocalDiskImage', 'lsbootEmbeddedLocalDiskImagePath', 'lsbootEmbeddedLocalLunImage',
            'lsbootIScsi', 'lsbootIScsiImagePath', 'lsbootLan', 'lsbootLanImagePath', 'lsbootLocalDiskImage',
            'lsbootLocalDiskImagePath', 'lsbootLocalHddImage', 'lsbootLocalLunImagePath', 'lsbootNvme', 'lsbootPolicy',
            'lsbootSan', 'lsbootSanCatSanImage', 'lsbootSanCatSanImagePath', 'lsbootStorage', 'lsbootUEFIBootParam',
            'lsbootUsbExternalImage', 'lsbootUsbFlashStorageImage', 'lsbootUsbInternalImage', 'lsbootVirtualMedia',
            'lsbootLocalStorage', 'lsmaintMaintPolicy', 'lsPower', 'lsRequirement', 'lsServer', 'lsServerExtension',
            'lstorageControllerDef', 'lstorageControllerModeConfig', 'lstorageControllerRef', 'lstorageDasScsiLun',
            'lstorageDiskGroupConfigPolicy', 'lstorageDiskGroupQualifier', 'lstorageDiskSlot',
            'lstorageDiskZoningPolicy', 'lstorageLocal', 'lstorageLocalDiskConfigRef', 'lstorageLogin',
